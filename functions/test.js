...
exports.CreateCustomerPortalSession = functions.https.onCall(
  async (data, context) => {
    try {
      ...
      const dbObj = await db.collection("users").doc(data);
...
exports.CreateCheckoutSession = functions.https.onCall(async (req, res) => {
  try {
    console.log(req);
    const dbObj = await db.collection("users").doc(req.uId);
    var user = await dbObj.get();
    var userData = user.data();
    var session;
    if ("customerId" in userData) {
 ...
 */
exports.RetriveCheckoutSession = functions.https.onCall(
  async (data, context) => {
    try {
      const dbObj = await db.collection("users").doc(data);
      var user = await dbObj.get();
      ...
      if (
        ...
        typeof userData.lastSessionID === "undefined"
      ) {
